...
function deSlugifyStr(str: string) {
  ...
  if (!str.includes("-") && !str.includes("_")) return capitalizeFirstLetter(str);

  // replaces '-' or '_ with ' '
  const StrWithDashesAndHypensReplacedWithSpaces = str.replace(/-/g, "  ").replace(/_/g, "  ");
...
function slugifyString(str: string) {
  if (typeof str !== "string")
    throw new TypeError(
      `Invalid value passed to slugifyString!: Expected string but received ${typeof str}`,
    );
